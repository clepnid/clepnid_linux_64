...
public class OpcionesModulosHttp {
	...
	public static int mes = Integer.valueOf(Integer.toString(c.get(Calendar.MONTH))) + 1,
			anyo = Integer.valueOf(Integer.toString(c.get(Calendar.YEAR)).substring(1)), tamanyoCodigo = 40;
	...
	public static boolean validar() {
		...
		if (codigo != null && codigo.length()!=tamanyoCodigo) {
			return false;
		}
		int contadorPar = 0, contadorImpar = 0;
		for (int i = 1; i < tamanyoCodigo; i = i + 2) {
			contadorImpar += getNumeroLetra(codigo.charAt(i));
		}
		for (int i = 0; i < tamanyoCodigo; i = i + 2) {
	...
	public static boolean validar(String codigo) {
		if (codigo.length()!=tamanyoCodigo) {
			return false;
		}
		int contadorPar = 0, contadorImpar = 0;
		for (int i = 1; i < tamanyoCodigo; i = i + 2) {
